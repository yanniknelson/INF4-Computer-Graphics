...
int main(int argc, char* argv[]){
	...
	int samples = (argc == 4) ? atoi(argv[3]) : 100; //get the number of samples
	int resolution = 0;
	...
	RayTracer::tonemap(pixelbuffer, camWidth, camHeight);

	std::printf("Output file: %s\n",outputFile);

	//write rendered scene to file (pixels RGB values must be in range 0255)
	PPMWriter::PPMWriter(camWidth, camHeight, pixelbuffer, outputFile);
