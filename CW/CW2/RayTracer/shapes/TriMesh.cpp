...
namespace rt{
	...
	TriMesh::TriMesh(std::string path, Vec3f position, float scale,Material* mat) {
		...
		std::cout << faces.size() << " Built Tree " << VolumeRoot->numOfShapes << std::endl;

		WorldBound = ObjectToWorld.TransformBounds3(ObjectBound);

		std::cout << "Trimesh Object Bounds " << ObjectBound.pMin << " " << ObjectBound.pMax << std::endl;
		std::cout << "Trimesh World Bounds " << WorldBound.pMin << " " << WorldBound.pMax << std::endl;
		std::cout << "World back to object " << WorldToObject.TransformPoint(WorldBound.pMin) << " " << WorldToObject.TransformPoint(WorldBound.pMax) << std::endl;
